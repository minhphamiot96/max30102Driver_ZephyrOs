                                         ...
                                         float *correl)
{
   ...
   uint32_t an_x_temp[BUFFER_SIZE]; //ir
   uint32_t an_y_temp[BUFFER_SIZE]; //red
   ...
   {
      ring_buf_get (ringBufIr, (uint8_t *)&dataIr, sizeof(dataIr));
      an_x_temp[k] = dataIr;
      f_ir_mean += dataIr;
      ring_buf_get (ringBufIr, (uint8_t *)&dataRed, sizeof(dataRed));
      an_y_temp[k] = dataRed;
      f_red_mean += dataRed;
   }
   ...
   {
      *ptr_x = an_x_temp[k] - f_ir_mean;
      *ptr_y = an_y_temp[k] - f_red_mean;
   }
